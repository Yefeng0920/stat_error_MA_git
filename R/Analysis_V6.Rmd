---
title: "Credibility crisis: quantitative analysis of the trustworthiness of science"
...
# functions
# custom functions used to compute replicability, prior probability, statistical power, magnitude error, and sign error.
...
Load the meta-analytic level point estimates and uncertainty.
...
#--------------------economics--------------------#
################################################
weightfunct(effect = dat_eco$mu_unadj, v = dat_eco$se_unadj^2, steps = c(.025,0.05), table = TRUE) 

# alternatively, we can use selmodel() from metafor
res_eco <- rma(yi = mu_unadj, sei = se_unadj, data = dat_eco)
selmodel(res_eco, type="stepfun", steps=c(.025,0.05))
...
weightfunct(effect = dat_env$mu_unadj, v = dat_env$se_unadj^2, steps = c(.025,0.05), table = TRUE)

################################################
#--------------------medicine--------------------#
################################################
# medical dataset is too large that both selmodel() and weightfunct() have issues in running selection model (due to the k*k matrix multiplication). So, we decided to split the dataset
...
selmod_med <- lapply(dat_chunks, function(chunk) {
  tryCatch({
    weightfunct(effect = chunk$mu_unadj, v = chunk$se_unadj^2, steps = c(.025,0.05), table = TRUE)
  }, error = function(e) {
...
weightfunct(effect = dat_psy$mu_unadj, v = dat_psy$se_unadj^2, steps = c(.025,0.05), table = TRUE)
...
We test the excess statistical significance. We further calculate The proportion of the research record that has been selected to be statistically significant, P(SSS), equals ESS/(1-Esig), where ESS is the difference between the observed proportion of statistically significant effects (Pss) and expected proportion assuming no selection for statistical significance (Esig). Details can be found at Tom's paper: Stanley T D, Doucouliagos H, Ioannidis J P A, et al. Detecting publication selection bias through excess statistical significance[J]. Research Synthesis Methods, 2021, 12(6): 776-795.
...
################################################
# test of excess significance
tes_eco <- tes(mu_unadj, I(se_unadj^2), data=dat_eco, test="binom") 
# calculate P(SSS)
...
################################################
# test of excess significance
tes_env <- tes(mu_unadj, I(se_unadj^2), data=dat_env, test="binom")
# calculate P(SSS)
...
################################################
# test of excess significance
tes_med <- tes(mu_unadj, I(se_unadj^2), data=dat_med, test="binom")
# calculate P(SSS)
...
################################################
# test of excess significance
tes_psy <- tes(mu_unadj, I(se_unadj^2), data=dat_psy, test="binom")
# calculate P(SSS)
...
```{r}
# medicine
p1 <- ggdensity(dat_med, x = "z_adj", y = "..density..", 
   #add = "median", 
   rug = TRUE, alpha = 0.5,
   color = brewer.pal(n = 12, name = "Set3")[5], # brewer.pal(n = 8, name = "Dark2")[1]
   fill = brewer.pal(n = 12, name = "Set3")[5]
   ) + 
  geom_vline(xintercept = quantile(dat_med$z_adj, probs = 0.25), linetype = "dashed", color="red") + 
  geom_vline(xintercept = quantile(dat_med$z_adj, probs = 0.5), linetype = "dashed", color="black")+
  geom_vline(xintercept = quantile(dat_med$z_adj, probs = 0.75), linetype = "dashed", color="blue") +
  labs(x = "Signal-noise-ratio", y = "Density", title = "Medicine") +
  #guides(color="none")
  theme_bw() + 
  theme(axis.text = element_text(size = 14, color = "black"),
        axis.title = element_text(size = 18, color = "black"),
        plot.title = element_text(size = 18)) + 
  scale_x_continuous(expand = c(0, 0.2), breaks = seq(0,10,2), labels = seq(0,10,2)) + coord_cartesian(xlim = c(0,10))

# economics
p2 <- ggdensity(dat_eco, x = "z_adj", y = "..density..", 
   rug = TRUE, alpha = 0.5,
   color = brewer.pal(n = 12, name = "Set3")[4], 
   fill = brewer.pal(n = 12, name = "Set3")[4]
   ) + 
  geom_vline(xintercept = quantile(dat_eco$z_adj, probs = 0.25), linetype = "dashed", color="red") + 
  geom_vline(xintercept = quantile(dat_eco$z_adj, probs = 0.5), linetype = "dashed", color="black")+
  geom_vline(xintercept = quantile(dat_eco$z_adj, probs = 0.75), linetype = "dashed", color="blue") +
  labs(x = "Signal-noise-ratio", y = "Density", title = "Economics") +
  theme_bw() + 
  theme(axis.text = element_text(size = 14, color = "black"),
        axis.title = element_text(size = 18, color = "black"),
        plot.title = element_text(size = 18)) + 
...
# environment
p3 <- ggdensity(dat_env, x = "z_adj", y = "..density..", 
   rug = TRUE, alpha = 0.5,
   color = brewer.pal(n = 12, name = "Set3")[3], 
   fill = brewer.pal(n = 12, name = "Set3")[3]
   ) + 
  geom_vline(xintercept = quantile(dat_env$z_adj, probs = 0.25), linetype = "dashed", color="red") + 
  geom_vline(xintercept = quantile(dat_env$z_adj, probs = 0.5), linetype = "dashed", color="black")+
  geom_vline(xintercept = quantile(dat_env$z_adj, probs = 0.75), linetype = "dashed", color="blue") +
  labs(x = "Signal-noise-ratio", y = "Density", title = "Enironment") +
  theme_bw() + 
  theme(axis.text = element_text(size = 14, color = "black"),
        axis.title = element_text(size = 18, color = "black"),
        plot.title = element_text(size = 18)) + 
...
# psychology
p4 <- ggdensity(dat_psy, x = "z_adj", y = "..density..", 
   rug = TRUE, alpha = 0.5,
   color = brewer.pal(n = 12, name = "Set3")[1], 
   fill = brewer.pal(n = 12, name = "Set3")[1]
   ) + 
  geom_vline(xintercept = quantile(dat_psy$z_adj, probs = 0.25), linetype = "dashed", color="red") + 
  geom_vline(xintercept = quantile(dat_psy$z_adj, probs = 0.5), linetype = "dashed", color="black")+
  geom_vline(xintercept = quantile(dat_psy$z_adj, probs = 0.75), linetype = "dashed", color="blue") +
  labs(x = "Signal-noise-ratio", y = "Density", title = "Psychology") +
  theme_bw() + 
  theme(axis.text = element_text(size = 14, color = "black"),
        axis.title = element_text(size = 18, color = "black"),
        plot.title = element_text(size = 18)) + 
...
png(filename = "figure2.png", width = 14, height = 12, units = "in", type = "windows", res = 400)
p1 + p2 + p3 + p4 + plot_layout(nrow = 2, ncol = 2) + plot_annotation(tag_levels = list(c('A', 'B', 'C', 'D'))) & theme(plot.tag = element_text(size = 18, face = "bold"))
dev.off()
```
...
rep.dist.eco <- ggplot(data = rep_eco, aes(x = pval, y = replicate)) +
  ...
  geom_ribbon(data = filter(rep_eco, pval <= 0.05 & pval >= 0.01), aes(pval, ymax=replicate, ymin=0), fill=brewer.pal(n = 12, name = "Set3")[4], alpha=0.2) +
  geom_ribbon(data = filter(rep_eco, pval <= 0.01 & pval >= 0.001), aes(pval, ymax=replicate, ymin=0), fill=brewer.pal(n = 12, name = "Set3")[4], alpha=0.5) +
  geom_ribbon(data = filter(rep_eco, pval <= 0.001), aes(pval, ymax=replicate, ymin=0), fill=brewer.pal(n = 12, name = "Set3")[4], alpha=1) + 
...
rep.dist.env <- ggplot(data = rep_env, aes(x = pval, y = replicate)) +
  geom_line(linetype="solid", color="black", linewidth = 0.5) +
  geom_ribbon(data = filter(rep_env, pval <= 0.05 & pval >= 0.01), aes(pval, ymax=replicate, ymin=0), fill=brewer.pal(n = 12, name = "Set3")[3], alpha=0.2) +
  geom_ribbon(data = filter(rep_env, pval <= 0.01 & pval >= 0.001), aes(pval, ymax=replicate, ymin=0), fill=brewer.pal(n = 12, name = "Set3")[3], alpha=0.5) +
  geom_ribbon(data = filter(rep_env, pval <= 0.001), aes(pval, ymax=replicate, ymin=0), fill=brewer.pal(n = 12, name = "Set3")[3], alpha=1) + 
  #geom_text(data = data.frame(x = c(2.27, 2.935, 4.645), label = c("Modest", "Strong", "Very Strong")), aes(x = x, y = 0.02, label = label), vjust = 0, size = 3, color = "black", alpha = 0.7) +
...
rep.dist.psy <- ggplot(data = rep_psy, aes(x = pval, y = replicate)) +
  geom_line(linetype="solid", color="black", linewidth = 0.5) +
  geom_ribbon(data = filter(rep_psy, pval <= 0.05 & pval >= 0.01), aes(pval, ymax=replicate, ymin=0), fill=brewer.pal(n = 12, name = "Set3")[1], alpha=0.2) +
  geom_ribbon(data = filter(rep_psy, pval <= 0.01 & pval >= 0.001), aes(pval, ymax=replicate, ymin=0), fill=brewer.pal(n = 12, name = "Set3")[1], alpha=0.5) +
  geom_ribbon(data = filter(rep_psy, pval <= 0.001), aes(pval, ymax=replicate, ymin=0), fill=brewer.pal(n = 12, name = "Set3")[1], alpha=1) + 
  #geom_text(data = data.frame(x = c(2.27, 2.935, 4.645), label = c("Modest", "Strong", "Very Strong")), aes(x = x, y = 0.02, label = label), vjust = 0, size = 3, color = "black", alpha = 0.7) +
...
# assembly
png(filename = "figure3.png", width = 14, height = 12, units = "in", type = "windows", res = 400)
...
df$discipline <- factor(df$discipline, levels = c("psychology", "environmental", "economics", "medicine"))
...
p_R <- ggplot(df, aes(x = R, y = discipline, fill = discipline)) +
  geom_density_ridges(color = "white", alpha = 0.5,
                      ...
                      scale = 1,
                      quantile_lines = TRUE) +
  scale_fill_manual(values = brewer.pal(n = 12, name = "Set3")[-2]) + 
  scale_x_continuous(limits = c(0, 1), breaks = seq(0, 1, by = 0.2),
                     minor_breaks=seq(0,1,0.05),expand = expansion(mult = c(0, 0.015)), labels = scales::percent_format(accuracy=1)) +
  scale_y_discrete(labels = c("Psychology", "Environment",  "Economics", "Medicine"), expand = expansion(mult = c(0.01, 0.25))) +
  guides(fill = "none") +
...
p_S <- ggplot(filter(df, S_adj<=0.1), aes(x = S_adj, y = discipline, fill = discipline)) +
  ...
  scale_fill_manual(values = brewer.pal(n = 12, name = "Set3")[-2]) +
  scale_x_continuous(limits = c(0, 0.02), breaks = seq(0, 0.02, by = 0.002),
                     minor_breaks=seq(0,0.02,0.001),expand = expansion(mult = c(0, 0.015)), labels = scales::percent_format(accuracy=0.1)) +
  scale_y_discrete(labels = c("Psychology", "Environment", "Economics", "Medicine"), expand = expansion(mult = c(0.01, 0.25))) +
  guides(fill = "none") +
  labs(x = "Sign error", y = "") +
  theme_bw() + 
...
# layout
png(filename = "figure4.png", width = 5, height = 10, units = "in", type = "windows", res = 400)
...
```{r}
# economics
p1 <- dat_eco %>% ggplot(aes(x = K)) +
  stat_dots(color = brewer.pal(n = 12, name = "Set3")[4], fill = brewer.pal(n = 12, name = "Set3")[4], size = 1, alpha = 0.5) + 
  scale_x_continuous(limits = c(0, 2000), breaks = seq(0, 2000, 400)) + 
  labs(x = "Number of studies per meta-analysis", title = "Economics") + 
  theme_bw() +
  theme(
  ...
  geom_vline(aes(xintercept = quantile(K, prob = 0.25)), color = "blue", linetype = "dashed", size = 0.5) + 
  geom_vline(aes(xintercept = quantile(K, prob = 0.50)), color = "black", linetype = "dashed", size = 0.5) + 
  geom_vline(aes(xintercept = quantile(K, prob = 0.75)), color = "red", linetype = "dashed", size = 0.5)

# environment
p2 <- dat_env %>% ggplot(aes(x = K)) +
  stat_dots(color = brewer.pal(n = 12, name = "Set3")[3], fill = brewer.pal(n = 12, name = "Set3")[3], size = 1, alpha = 0.5) + 
  scale_x_continuous(limits = c(0, 400), breaks = seq(0, 400, 50)) + 
  labs(x = "Number of studies per meta-analysis", title = "Environment") + 
  theme_bw() +
  theme(
    axis.text.y = element_blank(),  
    axis.ticks.y = element_blank(), 
...
# medicine
p3 <- dat_med %>% ggplot(aes(x = K)) +
  stat_dots(color = brewer.pal(n = 12, name = "Set3")[5], fill = brewer.pal(n = 12, name = "Set3")[5], size = 1, alpha = 0.5) + 
  scale_x_continuous(limits = c(0, 300), breaks = seq(0, 400, 50)) + 
  labs(x = "Number of studies per meta-analysis", title = "Medicine") + 
  theme_bw() +
  theme(
    axis.text.y = element_blank(),  
    axis.ticks.y = element_blank(), 
...
# psychology
p4 <- dat_psy %>% ggplot(aes(x = K)) +
  stat_dots(color = brewer.pal(n = 12, name = "Set3")[1], fill = brewer.pal(n = 12, name = "Set3")[1], size = 1, alpha = 0.5) + 
  scale_x_continuous(limits = c(0, 400), breaks = seq(0, 400, 50)) + 
  labs(x = "Number of studies per meta-analysis", title = "Psychology") + 
  theme_bw() +
  theme(
    axis.text.y = element_blank(),  
    axis.ticks.y = element_blank(), 
...
# assembly
png(filename = "figureS1.png", width = 10, height = 10, units = "in", type = "windows", res = 400)
p3 + p1 + p2 + p4 + plot_layout(nrow = 2, ncol = 2) + plot_annotation(tag_levels = list(c('A', 'B', 'C', 'D'))) & theme(plot.tag = element_text(size = 18, face = "bold"))
dev.off()
```
